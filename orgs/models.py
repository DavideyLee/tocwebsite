...
class Organization(models.Model):
    ...
    users = models.ManyToManyField('users.MyUser', related_name='orgs', blank=True)
    admins = models.ManyToManyField('users.MyUser', related_name='admin_orgs', blank=True)
    created_by = models.CharField(max_length=32, null=True, blank=True, verbose_name=_('Created by'))
    date_created = models.DateTimeField(auto_now_add=True, null=True, blank=True, verbose_name=_('Date created'))
    comment = models.TextField(max_length=128, default='', blank=True, verbose_name=_('Comment'))

    CACHE_PREFIX = 'TOC_ORG_{}'
    ROOT_ID_NAME = 'ROOT'
    ...
    def get_org_users(self, include_app=False):
        from users.models import MyUser
        if self.is_default():
            users = MyUser.objects.filter(orgs__isnull=True)
        ...
        if not include_app:
            users = users.exclude(role=MyUser.ROLE_APP)
        return users
